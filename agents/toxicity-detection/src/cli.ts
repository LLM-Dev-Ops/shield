 ...
 *   shield-agent toxicity-detection simulate --content "I hate you" --threshold 0.6
 *   shield-agent toxicity-detection inspect --execution-ref <uuid>
 *
 * @module toxicity-detection-agent/cli
 */

import { ToxicityDetectionAgent } from './agent.js';
import { AGENT_IDENTITY, type ToxicityCategory } from './types.js';
...
function parseArgs(args: string[]): CliOptions {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (arg) {
      ...
      case '--categories':
        options.categories = args[++i].split(',') as ToxicityCategory[];
        break;
...
Usage:
  shield-agent toxicity-detection <mode> [options]

Modes:
  test        Run toxicity detection on content
...
Options:
  ...
  --threshold, -t <num>      Detection threshold 0.0-1.0 (default: 0.7)
  --categories <cats>        Comma-separated categories to detect
  --help, -h                 Show this help message

Examples:
  shield-agent toxicity-detection test --content "You are an idiot"
  shield-agent toxicity-detection simulate --content "I hate you" -t 0.6
  shield-agent toxicity-detection inspect --execution-ref abc123
...
async function runSimulate(options: CliOptions): Promise<void> {
  ...
  if (options.verbose) {
    ...
    console.log(`  Categories: ${input.detect_categories?.join(', ') || 'all'}`);
    console.log('');
  }
...
function formatTextOutput(output: Record<string, unknown>, options: CliOptions): void {
  ...
  console.log('Toxicity Detection Results');
  console.log('==========================');
  console.log('');
  console.log(`Toxicity Detected: ${result.toxicity_detected ? 'Yes' : 'No'}`);
  ...
  if (options.verbose && (result.entities as unknown[]).length > 0) {
    ...
    for (const entity of result.entities as Record<string, unknown>[]) {
      console.log(`  - Category: ${entity.toxicity_category}`);
  ...
  if (options.verbose && (result.detected_categories as unknown[]).length > 0) {
    console.log('');
    console.log('Category Counts:');
    const categoryCounts = result.category_counts as Record<string, number>;
...
function formatTableOutput(output: Record<string, unknown>, options: CliOptions): void {
  ...
  // Header
  console.log('');
  console.log('| Category        | Start | End   | Confidence | Severity |');
  console.log('|-----------------|-------|-------|------------|----------|');

  // Rows
  for (const entity of entities) {
    const category = String(entity.toxicity_category).padEnd(15);
