...
import {
  AGENT_IDENTITY,
  type ToxicityDetectionInput,
  type ToxicityDetectionAgentOutput,
  type ToxicityDetectionResult,
  type ToxicityDetectedEntity,
  type RiskFactor,
...
import {
  ToxicityDetectionInput as ToxicityDetectionInputSchema,
  AgentError,
  type AgentErrorCode,
...
export class ToxicityDetectionAgent {
   ...
   */
  async detect(input: ToxicityDetectionInput): Promise<ToxicityDetectionAgentOutput> {
    ...
    try {
      ...
      // Create output
      const output: ToxicityDetectionAgentOutput = {
   ...
   */
  private validateInput(input: ToxicityDetectionInput): ToxicityDetectionInput {
    try {
   ...
   */
  private createResult(matches: PatternMatch[], durationMs: number): ToxicityDetectionResult {
    // Convert matches to entities (WITHOUT raw values)
    const entities: ToxicityDetectedEntity[] = matches.map(match => ({
    ...
    const detectedCategories = [...new Set(matches.map(m => m.pattern.category))] as ToxicityCategory[];
   ...
   */
  private createRiskFactors(matches: PatternMatch[]): RiskFactor[] {
    // Group matches by category
    const byCategory = new Map<ToxicityCategory, PatternMatch[]>();
    for (const match of matches) {
  ...
  private async persistDecisionEvent(
    input: ToxicityDetectionInput,
    output: ToxicityDetectionAgentOutput
  ): Promise<void> {
...
/**
 * Agent error type (matches AgentError schema)
 */
interface AgentErrorType {
  code: AgentErrorCode;
  message: string;
  agent?: typeof AGENT_IDENTITY;
