...
import { ModelAbuseDetectionInput, } from '@llm-shield/agentics-contracts';
import { createDetector } from './detector.js';
import { createClientFromEnv, createNoOpClient, } from './ruvector-client.js';
import { createTelemetryEmitterFromEnv, emitDetectionStarted, emitDetectionCompleted, emitDetectionError, emitBehavioralAnalysisCompleted, } from './telemetry.js';
/**
 ...
 */
function categorizeRequestRate(rate) {
    if (rate === undefined)
        return undefined;
    if (rate < 10)
        return 'low';
 ...
 */
function categorizeSessionRequests(count) {
    if (count === undefined)
        return undefined;
    if (count < 10)
        return 'few';
...
function buildDecisionEvent(input, result, durationMs) {
    const event = {
        ...
        telemetry: {
            ...
            session_request_bucket: categorizeSessionRequests(input.request_metadata?.session_request_count),
        },
    };
    return event;
}
...
export async function handleDetection(rawInput, config = {}) {
    ...
    const telemetryEmitter = config.telemetryEmitter ||
    ...
    try {
        ...
        const { entities, riskFactors, behavioralSummary } = detector.detect(input.content, {
            ...
            : undefined, input.historical_context
            ? {
        ...
        // Emit telemetry
        emitDetectionCompleted(telemetryEmitter, input.context.execution_ref, durationMs, result.abuse_detected, entities.length, detectedCategories, riskScore);
        emitBehavioralAnalysisCompleted(telemetryEmitter, input.context.execution_ref, behavioralSummary.appearsAutomated, behavioralSummary.abnormalRate, behavioralSummary.redFlagCount);
        // Build output
        const output = {
            agent: AGENT_IDENTITY,
            result,
        ...
        emitDetectionError(telemetryEmitter, input.context.execution_ref, 'INTERNAL_ERROR', errorMessage);
        const agentError = {
            code: 'INTERNAL_ERROR',
    ...
    async fetch(request) {
        ...
        try {
            ...
            return new Response(JSON.stringify(result), {
                status: isError ? result.code === 'INVALID_INPUT' ? 400 : 500 : 200,
                headers: {
                    'Content-Type': 'application/json',
