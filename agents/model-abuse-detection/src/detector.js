 ...
 */
import { MODEL_ABUSE_PATTERNS, BEHAVIORAL_THRESHOLDS, getPatternsForCategories, getThresholdsForCategories, } from './patterns.js';
/**
...
export class ModelAbuseDetector {
     ...
     */
    detect(content, config, metadata, historicalContext) {
        ...
        const behavioralMatches = this.detectBehavioralPatterns(applicableThresholds, metadata, historicalContext);
        // Analyze behavioral summary
        const behavioralSummary = this.analyzeBehavior(metadata, behavioralMatches);
        // Convert to entities
        const entities = this.convertToEntities(patternMatches, behavioralMatches, config);
        // Build risk factors
        const riskFactors = this.buildRiskFactors(patternMatches, behavioralMatches, behavioralSummary);
        return { entities, riskFactors, behavioralSummary };
    }
    /**
     * Detect content-based patterns
     */
    detectPatterns(content, patterns, config) {
        const matches = [];
        for (const pattern of patterns) {
            ...
            while ((match = regex.exec(content)) !== null) {
                ...
                const confidence = this.calculateConfidence(pattern.baseConfidence, config.sensitivity);
     ...
     */
    detectBehavioralPatterns(thresholds, metadata, historicalContext) {
        ...
        for (const threshold of thresholds) {
            ...
            if (actualValue !== undefined && actualValue >= threshold.threshold) {
                ...
                const confidence = Math.min(1.0, threshold.baseConfidence * Math.min(2, exceedanceRatio));
                matches.push({
                    threshold,
                    actualValue,
                    confidence,
     ...
     */
    analyzeBehavior(metadata, behavioralMatches) {
        const redFlags = [];
        ...
        const abnormalRate = (behavioralMatches ?? []).some((m) => m.threshold.unit === 'requests_per_minute');
        if (abnormalRate) {
     ...
     */
    convertToEntities(patternMatches, behavioralMatches, config) {
        const entities = [];
     ...
     */
    buildRiskFactors(patternMatches, behavioralMatches, behavioralSummary) {
        const factors = [];
        ...
        for (const [category, count] of categoryCounts) {
            factors.push({
                ...
                severity: this.getCategorySeverity(category, patternMatches),
     ...
     * Calculate confidence with sensitivity adjustment
     */
    calculateConfidence(baseConfidence, sensitivity) {
     ...
     */
    getCategorySeverity(category, matches) {
     ...
     */
    getCategoryConfidence(category, matches) {
        const categoryMatches = matches.filter((m) => m.pattern.category === category);
        if (categoryMatches.length === 0) {
            return 0;
        }
     ...
     */
    calculateRiskScore(entities, behavioralSummary) {
     ...
     */
    getDetectedCategories(entities) {
