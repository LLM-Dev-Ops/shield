...
import { getPatternsByCategory, getPairPatterns, getPasswordOnlyPatterns, getAuthHeaderPatterns, createCustomPattern, } from './patterns.js';
 ...
 */
function redactCredential(value, type) {
    if (value.length <= 4) {
 ...
 */
function detectCredentials(content, patterns, sensitivity, minPasswordLength) {
    const entities = [];
    const seen = new Set();
 ...
 */
function buildExposureSummary(entities) {
...
export async function handleDetection(rawInput, config = {}) {
    ...
    try {
        ...
        // Run detection
        const entities = detectCredentials(input.content, patterns, input.sensitivity ?? 0.5, input.min_password_length ?? 6);
        // Build result
        const detectedTypes = [...new Set(entities.map((e) => e.credential_type))];
        ...
        emitDetectionCompleted(telemetryEmitter, input.context.execution_ref, durationMs, result.credentials_detected, entities.length, detectedTypes, result.severity);
        // Return output
        const output = {
            agent: AGENT_IDENTITY,
            result,
