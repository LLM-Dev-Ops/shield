...
export class TelemetryEmitter {
     ...
     */
    emitInvocationStart(executionRef, contentLength, contentSource, contentType) {
        this.emit({
     ...
     */
    emitInvocationComplete(executionRef, durationMs, allowed, action, riskScore, violationCount, requiresHumanReview) {
        this.emit({
     ...
     */
    emitModerationDecision(executionRef, action, reason, confidence, requiresHumanReview) {
        this.emit({
     ...
     */
    emitViolationDetected(executionRef, category, patternId, confidence, severity, recommendedAction) {
    ...
    emitFromDecisionEvent(event) {
        this.emitInvocationComplete(event.execution_ref, event.duration_ms, event.outputs.allowed, event.outputs.action, event.outputs.risk_score, event.outputs.violation_count, event.outputs.requires_human_review);
    }
    /**
