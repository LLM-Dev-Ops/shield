...
import { ContentModerationAgentInput as ContentModerationAgentInputSchema, } from '@llm-shield/agentics-contracts';
import { getPatternsForCategories, MODERATION_CATEGORIES, } from './patterns.js';
...
export class ContentModerationAgent {
    ...
    async moderate(input) {
        ...
        this.telemetry.emitInvocationStart(input.context.execution_ref, input.content.length, input.context.content_source, input.content_type);
        try {
            ...
            const minConfidence = input.min_moderation_confidence ?? this.config.minModerationConfidence;
            ...
            if (this.config.detailedTelemetry) {
                for (const violation of violations) {
                    this.telemetry.emitViolationDetected(input.context.execution_ref, violation.category, violation.pattern_id ?? 'unknown', violation.confidence, violation.severity, violation.recommended_action);
                }
            }
            // Build risk factors
            ...
            const decision = this.makeModerationDecision(violations, input.moderation_rules, input.default_action ?? this.config.defaultAction, minConfidence, confidence, input.user_age_verified ?? false);
            // Emit moderation decision telemetry
            this.telemetry.emitModerationDecision(input.context.execution_ref, decision.action, decision.reason, decision.confidence, decision.requiresHumanReview);
            ...
            this.telemetry.emitInvocationComplete(input.context.execution_ref, durationMs, result.allowed, result.action, result.risk_score, violations.length, result.requires_human_review);
            return output;
        }
        catch (error) {
            ...
            this.telemetry.emitInvocationError(input.context.execution_ref, 'INTERNAL_ERROR', errorMessage);
            throw error;
        }
    }
    /**
    ...
    buildViolations(matches, sensitivity) {
    ...
    buildRiskFactors(matches, sensitivity) {
        ...
        for (const [category, categoryMatches] of byCategory) {
            ...
            const avgConfidence = categoryMatches.reduce((sum, m) => sum + m.pattern.confidence, 0) /
    ...
    calculateRiskScore(factors, sensitivity) {
        if (factors.length === 0)
            return 0;
        // Weighted average of factor contributions
        const totalContribution = factors.reduce((sum, f) => sum + f.score_contribution * f.confidence, 0);
        // Normalize and apply sensitivity
        const baseScore = Math.min(1, totalContribution / factors.length);
        return Math.min(1, baseScore * (0.5 + sensitivity * 0.5));
    }
    calculateConfidence(matches, sensitivity) {
        if (matches.length === 0)
            return 1; // High confidence in "safe" decision
    ...
    adjustConfidence(baseConfidence, sensitivity) {
    ...
    determineSeverity(score, violatedCategories) {
    ...
    makeModerationDecision(violations, moderationRules, defaultAction, minConfidence, overallConfidence, userAgeVerified) {
        // No violations = ALLOW
        ...
        if (moderationRules && moderationRules.length > 0) {
            ...
            for (const rule of sortedRules) {
                const matchingViolations = violations.filter((v) => v.category === rule.category && v.confidence >= rule.threshold);
    ...
    async persistDecisionEvent(input, output, durationMs, categoriesChecked) {
        ...
        if (result.success && result.event_id) {
            this.telemetry.emitPersistenceSuccess(input.context.execution_ref, result.event_id);
        }
        else {
            this.telemetry.emitPersistenceFailure(input.context.execution_ref, result.error || 'Unknown error');
        }
    }
}
/**
