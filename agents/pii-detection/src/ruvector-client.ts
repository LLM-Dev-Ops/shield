 ...
 *
 * @module pii-detection-agent/ruvector-client
 */

import type { PIIDetectionDecisionEvent, RuvectorClient, RuvectorClientConfig } from './types.js';
...
export function createRuvectorClient(config?: Partial<RuvectorClientConfig>): RuvectorClient {
  const finalConfig: RuvectorClientConfig = {
    ...DEFAULT_CONFIG,
    ...config,
  };
 ...
 */
class RuvectorClientImpl implements RuvectorClient {
   ...
   * Raw content, PII values, and sensitive data MUST NOT be included.
   */
  async persistDecisionEvent(event: PIIDetectionDecisionEvent): Promise<void> {
   ...
   * Validate event before sending
   */
  private validateEvent(event: PIIDetectionDecisionEvent): void {
    // Validate required fields
    if (!event.agent_id || event.agent_id !== 'pii-detection-agent') {
...
export function createDecisionEvent(params: {
  ...
  timestamp: string;
  inputsHash: string;
  outputs: PIIDetectionDecisionEvent['outputs'];
  confidence: number;
  constraintsApplied: PIIDetectionDecisionEvent['constraints_applied'];
  durationMs: number;
  telemetry?: PIIDetectionDecisionEvent['telemetry'];
