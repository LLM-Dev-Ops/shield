...
import {
  AGENT_IDENTITY,
  type PIIDetectionInput,
  type PIIDetectionAgentOutput,
...
export class PIIDetectionAgent {
   ...
   */
  async detect(input: PIIDetectionInput): Promise<PIIDetectionAgentOutput> {
    ...
    try {
      ...
      // Create output
      const output: PIIDetectionAgentOutput = {
   ...
   */
  private validateInput(input: PIIDetectionInput): PIIDetectionInput {
    try {
   ...
   */
  private createResult(matches: PatternMatch[], durationMs: number): PIIDetectionResult {
    // Convert matches to entities (WITHOUT raw values)
    const entities: PIIDetectedEntity[] = matches.map(match => ({
    ...
    const detectedTypes = [...new Set(matches.map(m => m.pattern.type))] as PIIType[];
  ...
  private createRiskFactors(matches: PatternMatch[]): RiskFactor[] {
    ...
    const byType = new Map<PIIType, PatternMatch[]>();
    for (const match of matches) {
  ...
  private async persistDecisionEvent(
    ...
    output: PIIDetectionAgentOutput
  ): Promise<void> {
