...
import { PII_PATTERNS, getPatternsByCountry } from './patterns.js';
import type {
  PIIType,
  PIICountry,
  Severity,
...
export class PIIDetector {
   ...
   */
  private filterPatterns(config: DetectionConfig): PIIPattern[] {
    return this.patterns.filter(pattern => {
      ...
      if (pattern.countries && pattern.countries.length > 0) {
        const hasMatchingCountry = pattern.countries.some(c =>
          config.countries.includes(c)
        );
   ...
   */
  private validate(value: string, pattern: PIIPattern): boolean {
   ...
   */
  private ssnAreaCheck(value: string): boolean {
    const digits = value.replace(/\D/g, '');
    if (digits.length !== 9) {
      return false;
    }
   ...
   */
  private formatCheck(value: string, pattern: PIIPattern): boolean {
    switch (pattern.type) {
      case 'email':
   ...
   */
  private validateIPAddress(value: string): boolean {
    // IPv4 validation
    if (value.includes('.')) {
   ...
   */
  private getConfidenceThreshold(sensitivity: number): number {
