...
import { PromptInjectionDetectionInput as PromptInjectionDetectionInputSchema, } from '@llm-shield/agentics-contracts';
import { DETECTION_PATTERNS, CATEGORIES, getPatternsForCategories, } from './patterns.js';
...
export class PromptInjectionDetectionAgent {
    ...
    async detect(input) {
        ...
        try {
            ...
            this.telemetry.emitInvocationComplete(input.context.execution_ref, durationMs, output.result.threats_detected, output.result.risk_score, output.result.pattern_match_count);
            return output;
        }
        catch (error) {
            const durationMs = performance.now() - startTime;
    ...
    // Private Methods
    // ===========================================================================
    getActivePatterns(categories) {
        if (!categories || categories.length === 0) {
            return DETECTION_PATTERNS;
        }
        // Map string categories to Category type
        const validCategories = categories.filter((c) => Object.values(CATEGORIES).includes(c));
    ...
    buildEntities(matches, sensitivity) {
    ...
    async persistDecisionEvent(input, output, durationMs) {
