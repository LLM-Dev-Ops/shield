 ...
 */
import type { AgentIdentity, DecisionEvent } from '@llm-shield/agentics-contracts';
/**
 * Telemetry event types
 */
export type TelemetryEventType = 'agent.invocation.start' | 'agent.invocation.complete' | 'agent.invocation.error' | 'agent.detection.pattern_match' | 'agent.persistence.success' | 'agent.persistence.failure';
/**
...
export declare class TelemetryEmitter {
     ...
     */
    emitInvocationComplete(executionRef: string, durationMs: number, threatsDetected: boolean, riskScore: number, patternMatchCount: number): void;
    /**
     ...
     */
    emitPatternMatch(executionRef: string, category: string, patternId: string, confidence: number): void;
    /**
     ...
     */
    emitFromDecisionEvent(event: DecisionEvent): void;
    /**
