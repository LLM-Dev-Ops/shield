 ...
 */
import type { AgentIdentity, SafetyBoundaryDecisionEvent, EnforcementAction } from '@llm-shield/agentics-contracts';
/**
 * Telemetry event types
 */
export type TelemetryEventType = 'agent.invocation.start' | 'agent.invocation.complete' | 'agent.invocation.error' | 'agent.enforcement.decision' | 'agent.enforcement.violation' | 'agent.persistence.success' | 'agent.persistence.failure';
/**
...
export declare class TelemetryEmitter {
     ...
     */
    emitInvocationStart(executionRef: string, contentLength: number, contentSource: string): void;
    /**
     * Emit invocation complete event
     */
    emitInvocationComplete(executionRef: string, durationMs: number, allowed: boolean, action: EnforcementAction, riskScore: number, violationCount: number): void;
    /**
     ...
     */
    emitEnforcementDecision(executionRef: string, action: EnforcementAction, reason: string, confidence: number): void;
    /**
     * Emit violation detection event (for detailed metrics)
     */
    emitViolationDetected(executionRef: string, category: string, patternId: string, confidence: number, severity: string): void;
    /**
