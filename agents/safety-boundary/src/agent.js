...
import { SafetyBoundaryAgentInput as SafetyBoundaryAgentInputSchema, } from '@llm-shield/agentics-contracts';
import { getPatternsForCategories, SAFETY_CATEGORIES, } from './patterns.js';
...
export class SafetyBoundaryAgent {
    ...
    async enforce(input) {
        ...
        this.telemetry.emitInvocationStart(input.context.execution_ref, input.content.length, input.context.content_source);
        try {
            ...
            const minConfidence = input.min_enforcement_confidence ?? this.config.minEnforcementConfidence;
            ...
            if (this.config.detailedTelemetry) {
                for (const violation of violations) {
                    this.telemetry.emitViolationDetected(input.context.execution_ref, violation.category, violation.pattern_id ?? 'unknown', violation.confidence, violation.severity);
                }
            }
            // Build risk factors
            ...
            const decision = this.makeEnforcementDecision(violations, input.policy_rules, input.default_action ?? this.config.defaultAction, minConfidence, confidence);
            // Emit enforcement decision telemetry
            this.telemetry.emitEnforcementDecision(input.context.execution_ref, decision.action, decision.reason, decision.confidence);
            ...
            this.telemetry.emitInvocationComplete(input.context.execution_ref, durationMs, result.allowed, result.action, result.risk_score, violations.length);
            return output;
        }
        catch (error) {
    ...
    makeEnforcementDecision(violations, policyRules, defaultAction, minConfidence, overallConfidence) {
        // No violations = ALLOW
