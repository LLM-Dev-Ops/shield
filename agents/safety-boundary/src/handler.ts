...
import { SafetyBoundaryAgent, createAgent, AGENT_IDENTITY } from './agent.js';
 ...
 */
async function handleEnforce(request: EdgeRequest): Promise<EdgeResponse> {
...
async function handleCliTest(
  ...
  const input = {
    ...
    enforce_categories: cli.config?.categories as string[] | undefined,
    default_action: (cli.config?.default_action as 'BLOCK' | 'ALLOW') ?? 'BLOCK',
  };
...
function formatAsText(data: unknown, verbose: boolean): string {
  const output = data as SafetyBoundaryAgentOutput;
...
function handleInfo(): EdgeResponse {
  return jsonResponse({
    ...
    description:
      'Enforces safety boundaries by evaluating content against configurable safety policies and making ALLOW/BLOCK enforcement decisions.',
    endpoints: [
